...
The alternative—maintaining separate planning scenes for each robot and synchronizing collision checks across a network boundary—would require:
...
## Visualizing and Debugging the Planning Scene
...
This returns the complete planning scene state: all link geometries, all collision objects, the ACM, padding values—everything the planner sees.

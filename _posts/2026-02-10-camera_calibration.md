...
Check out the `vision` branch in the repo for all the code to do your own calibration

## The Two Faces of Camera Parameters
Camera calibration splits into two distinct problems:
**Intrinsic parameters** describe the camera itself—its internal geometry. Think focal length, sensor dimensions, optical center, and lens distortion. These stay constant unless you swap lenses or change zoom.
**Extrinsic parameters** describe where the camera sits in the world—its position and orientation relative to the scene. These change every time the camera moves.
...
You get back `K` and distortion coefficients. Save these—they're reusable until hardware changes.
### Step 2: Undistort Images
Apply distortion correction using the coefficients from calibration. Now straight lines are actually straight.
### Step 3: Estimate Pose
Feed undistorted image points, known 3D coordinates, and `K` into `solvePnP`. It returns rotation `R` and translation `t`—your camera's pose.
### Step 4: Validate
...
- **Reprojection errors > 1 pixel** (good calibration should be < 0.5px)

...
**Note**
Collision geometry is defined separately from visual geometry because physics engines and motion planners check for collisions thousands of times per second. running those checks against high-polygon visual meshes would be too slow. Instead, Ned2's URDF visuals use COLLADA (.dae) files which carry color and texture while collision uses the corresponding STL file for the same link.
...
![Mesh](/mars_ned2/assets/images/mesh.png){: .align-center}
...
Validate and visualise before deployment:
...
| **Xacro** | Parameterised URDF (variables, macros, math) | Robots with repeated structures; maintainability | XML + macro language |
...
TF also returns transforms from past timestamps, which is critical for hand-eye calibration and sensor fusion. You can ask: "Where was Ned2's wrist when this camera frame arrived?" This temporal query is what makes multi-sensor systems coherent.
...
We'll come back to **transport frames** in detail at a later post.
...
**Mesh scale errors**: Ned2's meshes must align with kinematic dimensions exactly. A mesh scaled 0.1 units off creates a divergence between what MoveIt2 sees and what actually moves. Use `urdf_to_graphviz` to visualise the tree and spot scaling issues early.
...
the static URDF description becomes the synchronised model representing with hardware.

![mesh_n_collsion](/mars_ned2/assets/images/collision.png){: .align-center}

...
# Fuducial markers vs AI Pose Estimation

Why classical pose estimation failed in practice and how ArUco fiducial markers solved the problem for MARS. 
...
the task doesn't require recognising _arbitrary_ objects; the objects are known beforehand. They're items with specific dimensions. They can be marked.
...
- Camera 2 detects the same marker at (0.2, 0.2, 0.1) *in its own frame* (moving with arm_2)

These are **different coordinate systems**. Camera 1's "0.3m to the right" is  different space than Camera 2's "0.3m to the right"

The cameras cannot agree on absolute position because they're mounted on moving joints. 
...
Even though cameras disagree on absolute position, **detection itself is accurate**:
...
Camera intrinsics from earlier (K, distortion) matter for pose estimation accuracy **within each camera's frame**. Without calibration: position estimates are systematically wrong (error grows with distance). With calibration: accurate to ±1-2 cm at 30cm distance.
...
- **Rotation ambiguity**: Single flat marker can't distinguish 180° rotation around vertical. **Solution**: not needed, as pick operations only care about XY position, not Z-rotation
...
ArUco trades away generality for determinism, which is exactly what you need in a controlled, multi-robot environment.

---

## Summary

For dual-arm systems with known objects, ArUco markers provide **robust, deterministic detection** from each camera independently. The cameras will disagree on absolute position (they're on moving arms), which is expected. Planning handles the disagreement by reading both camera poses and transforming them using current joint states.

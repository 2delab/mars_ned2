...
**Asynchronous mode**: both arms plan and execute independently, 6-DOF each. No cross-arm collision checking. This mode delivers 28.6% cycle time reduction in parallel pick-and-place compared to sequential single-arm execution. Use this when the task naturally partitions into separate left and right work zones.
...
MARS delivers what we set out to do: namespace-isolated dual-arm coordination on the NED2 with two modes and planning-time safety. The system is validated, the limitations are clearly mapped, and the architecture extends naturally to richer problems (three arms, dynamic obstacles, adaptive safety margins).
